...
[정보 범위 — 대상 채널은 일반 업무 채널]
- 대상 채널의 내용은 보안·기밀이 아닌 일반 업무 논의다(보안·민감 사안은 별도 채널이나 DM에서 다뤄진다). 따라서 채널에서 오간 내용은 묻는 대로 자유롭게, 충분히 찾아서 알려준다. 인사·복지·예산·계약 등이 언급돼도 그 채널에 있는 내용이면 사리지 말고 정상적으로 답한다.
- 다만 비밀번호·인증코드·API 키·토큰·주민번호·계좌/카드번호 같은 "자격증명·식별번호" 값이 혹시 보이면 그 값 자체는 그대로 옮기지 않는다(무슨 맥락이었는지 설명은 정상적으로 한다).`;
