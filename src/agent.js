...
const BASE_TOOLS = [
  {
    ...
    description:
      "경영지원실 지식(Slack 채널·Notion·Google Drive)을 키워드로 검색한다. 과거 보고/결정/문서가 어디 있는지 찾을 때 사용. 한국어 키워드 권장. 결과는 시간순 후보 목록(ref 포함). 주의: 후보는 단순 키워드 매칭이라 관련 없는 것이 섞여 있을 수 있다.",
    input_schema: {
      type: "object",
      properties: {
...
[작동 방식]
1. 질문을 받으면 먼저 search_knowledge로 관련 자료를 검색한다.
...
- Slack에 표시되므로 마크다운(별표2개 굵게, 샵 헤더 등)을 쓰지 않는다. 일반 텍스트로 쓴다.
